...
test('checkStream', async () => {
  ...
  let res = await ssri.checkStream(stream, sri)
  assert.deepStrictEqual(res, meta, 'Stream data successfully verified')
  assert.ok(streamEnded, 'source stream ended')

  res = await ssri.checkStream(
    fileStream(),
    `sha512-${hash(TEST_DATA, 'sha512')}`
  )
  assert.deepStrictEqual(res, meta, 'Accepts string SRI')

  res = await ssri.checkStream(fileStream(), {
    algorithm: 'sha512',
    digest: hash(TEST_DATA, 'sha512'),
  })
  assert.deepStrictEqual(res, meta, 'Accepts Hash-like SRI')

  res = await ssri.checkStream(fileStream(), `sha512-${hash(TEST_DATA, 'sha512')}`, { single: true })
  assert.deepStrictEqual(res, meta, 'Process successfully with single option')
